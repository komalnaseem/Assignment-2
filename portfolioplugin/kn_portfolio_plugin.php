...
function create_post_type() {			/* https://codex.wordpress.org/Post_Types - Explains the functionality to create custom post types */
  register_post_type( 'kn_courses', 	/* Add "kn_courses" Custom Post Type to wordpress. https://codex.wordpress.org/Function_Reference/register_post_type Code referenced in order to create the custom post type. This function says to create and register the post type written in the array of the function. */
    array(
      'labels' => array(
        'name' => __( 'Courses' ),			  /* This name is displayed as a plural name in wordpress admin menu */
        'singular_name' => __( 'Course' ),	  /* This name is displayed as a singular name in wordpress admin menu */	
	  ...
	  'menu_icon' => 'dashicons-welcome-learn-more',     /* !!!Extra Functionality!!! - Changed the default custom post type icon to a graduation hat icon making it relevant to the courses custom post type. https://developer.wordpress.org/resource/dashicons/#welcome-learn-more - This link provides the code for menu_icon. */
	  'supports' => array (
 ...
 * This sets the path of the widget file by using a relative path. The function plugin_dir_path gets the path to this file and then I concatenate '/inc/kn_widget.php' to give the full
 * path. This ensures that when this plugin is installed by any user on their site, it will work because of relative path used.
 */
require plugin_dir_path( __FILE__ ) . '/inc/kn_widget.php'; 

/* 
* Shortcode to display Course taken at which school and it will include the link to the school website 
*/	
...
add_shortcode('schoollink','schoollink');				/* This adds a hook for the shortcode tag, schoollink */
